...
const PAY_VA_API = 'http://10.1.137.220:8772/pay/';
const GET_BY_ID_API = 'http://10.1.137.220:8772/get/';
...
export class DetailStatusLelangComponent implements OnInit {
  ...
  payVA() {
    const id = String(this.route.snapshot.paramMap.get('id'));
    this.http.put<any>(PAY_VA_API + id, this.httpOptions_base).subscribe(
      (isi) => {
